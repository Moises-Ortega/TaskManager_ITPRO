    ...
    <div class="flex justify-center">
            ...
            <form action="{{ route('groups.update', $group) }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full bg-transparent border border-purple-500/40 rounded-lg p-3 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-purple-400 transition duration-300">{{ $group->description }}</textarea>
                </div>
                    ...
                    <select name="category_id" id="category_id"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" class="bg-indigo-900"
                                {{ $group->category_id == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="priority_id" id="priority_id"
                        ...
                        @foreach ($priorities as $priority)
                            <option value="{{ $priority->id }}" class="bg-indigo-900"
                                {{ $group->priority_id == $priority->id ? 'selected' : '' }}>
                                {{ $priority->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div id="deleteModal" class="fixed inset-0 hidden bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="bg-white/10 border border-white/20 p-6 rounded-2xl shadow-xl text-center text-white max-w-sm">
                <h2 class="text-xl font-semibold mb-4">¿Eliminar este grupo?</h2>
                <p class="text-sm text-purple-200 mb-6">Esta acción no se puede deshacer. También se eliminarán sus tareas asociadas.</p>
                ...
                <div class="flex justify-center gap-4">
                    ...
                    <form action="{{ route('groups.destroy', $group) }}" method="POST" id="deleteForm">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
